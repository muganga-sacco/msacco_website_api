...
const create = async (req, res, next) => {
  try {
    const {
      ...
      key_deliverables, skills_competencies, personal_attributes,
      application_procedures,
      salary_range, deadline, max_age,
    } = req.body;
    ...
    const result = await query(
      `INSERT INTO careers (
        ...
        key_deliverables, skills_competencies, personal_attributes,
        application_procedures,
        salary_range, deadline, max_age, created_by
       )
      ...
      [
        ...
        JSON.stringify(key_deliverables || []),
        JSON.stringify(skills_competencies || []),
        JSON.stringify(personal_attributes || []),
        application_procedures || null,
        salary_range || null,
        deadline || null,
        max_age || null,
        req.user.id,
...
const update = async (req, res, next) => {
  try {
    const {
      ...
      key_deliverables, skills_competencies, personal_attributes,
      application_procedures,
      salary_range, deadline, max_age, is_active,
    } = req.body;
    ...
    const result = await query(
      `UPDATE careers SET
        ...
        key_deliverables       = COALESCE($8,  key_deliverables),
        skills_competencies    = COALESCE($9,  skills_competencies),
        personal_attributes    = COALESCE($10, personal_attributes),
        application_procedures = COALESCE($11, application_procedures),
        salary_range           = COALESCE($12, salary_range),
        deadline               = COALESCE($13, deadline),
        max_age                = COALESCE($14, max_age),
      ...
      [
        ...
        key_deliverables    ? JSON.stringify(key_deliverables)    : null,
        skills_competencies ? JSON.stringify(skills_competencies) : null,
        personal_attributes ? JSON.stringify(personal_attributes) : null,
        application_procedures ?? null,
        salary_range, deadline, max_age, is_active,
        req.params.id,
      ]
